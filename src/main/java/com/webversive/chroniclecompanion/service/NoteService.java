...
public class NoteService {
    ...
    public GeneralSession createNote(String username, String campaignId, NoteDTO note)
            throws UnauthorizedException {
        if (accountSqlService.isDungeonMaster(campaignId, username)) {
            if (notesSqlService.createNote(username, campaignId, note)) {
                return getNotesForCampaign(username, campaignId);
            }
            return null;
        }
    ...
    public NoteDTO getNoteById(String username, String campaignId, String noteId)
            throws UnauthorizedException {
        boolean authorized = accountSqlService.isDungeonMaster(campaignId, username);
