import { KINDRED, MORTAL, ANIMAL, GHOUL, WEREWOLF, BLOOD_SORCERY_RITUALS, TREMERE, CHARACTER_ATTRIBUTES, CHARACTER_SKILLS, LIMITED_SHEET } from './vampire.js';
import { addDiscipline, addPowers, addRitual, getDetailStr, updateTrackers, getStatValue, getSpecialty } from './vampire-settings.js';
...
function getAddSheetAttributes( being ) {
	...
	for ( let attributesType in CHARACTER_ATTRIBUTES ) {
		...
		if ( LIMITED_SHEET.includes( being ) ) {
			...
			for ( let attribute in CHARACTER_ATTRIBUTES[attributesType].attributes ) {
...
function validAdvAndFlaws( advAndFlaws ) {
	for ( let adv in advAndFlaws.advantages ) {
		if ( advAndFlaws.advantages[adv].level == 0 ) {
			return false;
		}
	}
	for ( let flaw in advAndFlaws.flaws ) {
...
function populateCharacterSheetSettings( characterData ) {
	$( '#character_name__wrapper input' ).val( characterData['name'] );
	...
	for ( let index in characterData.sheet.disciplines ) {
	...
	for ( let attrType in characterData.sheet.attributes ) {
		for ( let attr in characterData.sheet.attributes[attrType] ) {
	...
	for ( let skill in characterData.sheet.skills ) {
...
function populateLocationForm( locationData ) {
	$( '#location_add_name' ).val( locationData.name );
...
export { populateCharacterSheetSettings, populateLocationForm, serializeSheet, serializeLocation };
