...
function addSuperficial( trackerId, isReroll ) {
	...
	let characterBioElem = trackerElem.parent().parent().parent().parent().find( '.characterName' );
	...
	} else if ( addAggravatedDmg ) {
		// let newSuperficial = superficial - superficialChange;
		// let newAggravated = aggravated + superficialChange;

		updateCharacterValue( characterId, VALUE_TYPE_TRACKERS, trackerType, DAMAGE_SUPERFICIAL, superficialChange, true);
		updateCharacterValue( characterId, VALUE_TYPE_TRACKERS, trackerType, DAMAGE_AGGRAVATED, superficialChange, false);
	}
}
